...
static inline void sec_exec_devirt(struct kvm_vcpu *vcpu)
{
	static_call(kvm_x86_sec_exec_devirt)(vcpu);
}

static inline void kvm_set_cr3_exit(struct kvm_vcpu *vcpu, bool do_exit)
{
	static_call(kvm_x86_set_cr3_exit)(vcpu, do_exit);
}


static inline void kvm_set_ept(struct kvm_vcpu *vcpu, bool enable_ept)
{
	static_call(kvm_x86_set_ept)(vcpu, enable_ept);
}

void kvm_sec_exec_devirt_cpus(struct kvm *kvm) {
	int i;
	struct kvm_vcpu *vcpu;

	kvm_for_each_vcpu(i, vcpu, kvm)
		sec_exec_devirt(vcpu);
}
					       ...
					       struct kvm_vcpu_events *events)
{
	...
	events->devirt_user_cr3 = vcpu->arch.devirt_user_cr3;
	events->nmi.pad = 0;
...
int kvm_emulate_hypercall(struct kvm_vcpu *vcpu)
{
	...
	case KVM_HC_DO_DEVIRT_CR3: {
		unsigned long new_cr3;

		vcpu->arch.devirt_user_cr3 = (a0 | 1UL << 13);
		new_cr3 = (vcpu->arch.devirt_user_cr3 & ~X86_CR3_PCID_NOFLUSH) >> 12;
		new_cr3 = vcpu->kvm->gfn_to_hfn[new_cr3] << 12;
		new_cr3 |= (vcpu->arch.devirt_user_cr3 &
				(X86_CR3_PCID_NOFLUSH | X86_CR3_PCID_MASK));
		kvm_set_cr3(vcpu, new_cr3);
		kvm_set_cr3_exit(vcpu, true);
		kvm_set_ept(vcpu, false);
		ret = 0;
		break;
	}
	case KVM_HC_UNDO_DEVIRT_CR3: {
		unsigned long new_cr3;

		new_cr3 = (vcpu->arch.devirt_user_cr3 & ~(7UL << 11)) | 1UL << 63;
		kvm_set_ept(vcpu, true);
		kvm_set_cr3(vcpu, new_cr3);
		vcpu->arch.devirt_user_cr3 = 0;
		kvm_set_cr3_exit(vcpu, false);
		ret = 0;
		break;
	}
	case KVM_HC_INIT_DEVIRT:
		kvm_sec_exec_devirt_cpus(vcpu->kvm);
		break;
	default:
		ret = -KVM_ENOSYS;
		break;
