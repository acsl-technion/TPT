		...
		struct kvm *kvm)
{
	...
	case KVM_CPUID_FEATURES:
		...
		if (devirt_enabled) {
			pr_info("devirt create\n");
			entry->eax |= (1 << KVM_FEATURE_DEVIRT);
		} else {
			pr_info("no devirt\n");
		}
...
static int do_cpuid_func(struct kvm_cpuid_array *array, u32 func,
			 unsigned int type, struct kvm *kvm)
{
	if (type == KVM_GET_EMULATED_CPUID)
		return __do_cpuid_func_emulated(array, func);

	return __do_cpuid_func(array, func, kvm);
}
			  ...
			  unsigned int type, struct kvm *kvm)
{
	...
	r = do_cpuid_func(array, func, type, kvm);
	if (r)
		return r;
			    ...
			    unsigned int type, struct kvm *kvm)
{
	...
	for (i = 0; i < ARRAY_SIZE(funcs); i++) {
		r = get_cpuid_func(&array, funcs[i], type, kvm);
		if (r)
			goto out_free;
	}
