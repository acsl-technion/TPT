...
void vmx_update_exception_bitmap(struct kvm_vcpu *vcpu)
{
	...
	if (is_guest_mode(vcpu))
		...
		if (vcpu->enable_ept && (eb & (1u << PF_VECTOR))) {
			/*
...
static bool update_transition_efer(struct vcpu_vmx *vmx)
{
	 ...
	 */
	if (cpu_has_load_ia32_efer() ||
	    (vmx->vcpu.enable_ept && ((vmx->vcpu.arch.efer ^ host_efer) & EFER_NX))) {
		if (!(guest_efer & EFER_LMA))
			guest_efer &= ~EFER_LME;
...
void vmx_set_cr0(struct kvm_vcpu *vcpu, unsigned long cr0)
{
	...
	if (vcpu->enable_ept && !is_unrestricted_guest(vcpu)) {
		/*
			     ...
			     int root_level)
{
	...
	} else if (vcpu->arch.devirt_user_cr3) {
...
void vmx_set_cr3_exit(struct kvm_vcpu *vcpu, bool do_exit)
{
	struct vcpu_vmx *vmx = to_vmx(vcpu);
	if (do_exit)
		exec_controls_setbit(vmx, CPU_BASED_CR3_STORE_EXITING);
	else
		exec_controls_clearbit(vmx, CPU_BASED_CR3_STORE_EXITING);
}
...
void vmx_sec_exec_devirt(struct kvm_vcpu *vcpu)
{
	struct vcpu_vmx *vmx = to_vmx(vcpu);
	secondary_exec_controls_clearbit(vmx, SECONDARY_EXEC_ENABLE_PML);
	secondary_exec_controls_clearbit(vmx, SECONDARY_EXEC_MODE_BASED_EPT_EXEC);
	secondary_exec_controls_clearbit(vmx, SECONDARY_EXEC_UNRESTRICTED_GUEST);
}

void vmx_set_ept(struct kvm_vcpu *vcpu, bool l_enable_ept)
{
	struct vcpu_vmx *vmx = to_vmx(vcpu);
	if (l_enable_ept) {
		vcpu->enable_ept = true;
		secondary_exec_controls_setbit(vmx, SECONDARY_EXEC_ENABLE_EPT);
	} else {
		vcpu->enable_ept = false;
...
static int handle_exception_nmi(struct kvm_vcpu *vcpu)
{
	...
	if (is_page_fault(intr_info)) {
		...
		if (vcpu->enable_ept && !vcpu->arch.apf.host_apf_flags) {
			/*
...
static int handle_cr(struct kvm_vcpu *vcpu)
{
	...
	case 1: /*mov from cr*/
		...
		case 3:
			...
			if (vcpu->arch.devirt_user_cr3)
				val = vcpu->arch.devirt_user_cr3;
			else
...
static struct kvm_x86_ops vmx_x86_ops __initdata = {
	...
	.set_cr4 = vmx_set_cr4,
	.set_cr3_exit = vmx_set_cr3_exit,
	.set_ept = vmx_set_ept,
	.sec_exec_devirt = vmx_sec_exec_devirt,
