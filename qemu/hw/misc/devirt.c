...
#include "hw/misc/devirt.h"
#include "qom/object.h"
#include "sys/ioctl.h"
#include "sys/stat.h"

int kvm_vm_ioctl(KVMState *s, int type, ...);
#define PCI_VENDOR_ID_IVSHMEM   0x10ef
                             ...
                             uint64_t val, unsigned size)
{
    ...
    switch (addr)
    {
        ...
        case DOORBELL:
            ...
            break;
        case REGISTER_CR3:
            printf("Registered cr3 %lx\n", val);
            break;
        case DEREGISTER_CR3:
            printf("DE-registered cr3 %lx\n", val);
            break;
        default:
                                ...
                                unsigned size)
{
    ...
    switch (addr)
    {
        ...
        case TRUE_SIZE:
	    ret = (uint32_t)memory_region_size(&s->hostmem->mr);
            break;
...
static void ivshmem_common_realize(PCIDevice *dev, Error **errp)
{
    ...
    char buf[64];
    struct stat sb;
    KVMState *kvm_s = KVM_STATE(current_accel());
    ...
    kvm_vm_ioctl(kvm_s, _IO(0xAE,  0x49));
    sprintf(buf, "/proc/kvm-%d/gfn_to_hfn", getpid());
    stat(buf, &sb);
    printf("file %s\n", buf);
    printf("%lu\n", sb.st_size);

    memory_region_init_io(&s->ivshmem_mmio, OBJECT(s), &ivshmem_mmio_ops, s,
                          "devirt-mmio", IVSHMEM_REG_BAR_SIZE);
    ...
    object_property_set_str(obj, "mem-path", buf, &error_fatal);
    //object_property_set_int(obj, "size", 1UL * 1024 * 1024 * 1024, &error_fatal);
    object_property_set_int(obj, "size", sb.st_size, &error_fatal);
    object_property_add_child(qdev_get_machine(), "gfn_to_hfn", obj);
    ...
    memory_region_set_devirt(s->ivshmem_bar2);
