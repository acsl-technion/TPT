				   ...
				   unsigned long address, pte_t *ptep)
{
	...
	if (huge_ptep_set_access_flags(address, ptep, entry, 1))
			...
			unsigned long address, unsigned int flags)
{
	...
	entry = pte_mkyoung(entry);
	if (huge_ptep_set_access_flags(haddr, ptep, entry,
			    ...
			    struct page **pagep)
{
	...
	(void)huge_ptep_set_access_flags(dst_addr, dst_pte, _dst_pte,
