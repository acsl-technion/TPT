...
static void dump_pagetable(unsigned long address, int devirt)
{
	pgd_t *base;
	pgd_t *pgd;
	p4d_t *p4d;
	...
	if (!devirt)
		base = __va(read_cr3_pa() | 1UL << 13);
	else if (devirt == 1)
		base = __va(read_cr3_pa());
	else if (devirt == 2)
		base = __va(read_cr3_pa() | 1UL << 12);
		...
		unsigned long address, struct task_struct *tsk)
{
	...
	dump_pagetable(address, 0);
	dump_pagetable(address, 1);
	dump_pagetable(address, 2);
