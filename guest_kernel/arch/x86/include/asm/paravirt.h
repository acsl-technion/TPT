...
static inline int ptep_set_access_flags(unsigned long address, pte_t *ptep,
				 pte_t entry, int dirty)
{
	int ret;
	ret =  PVOP_CALL4(int, mmu.ptep_set_access_flags, address,
			ptep, native_pte_val(entry), dirty);
	return ret;
}
...
{
	int ret;
	ret = PVOP_CALL3(int, mmu.ptep_test_and_clear_young, vma, addr,
			ptep);
	return ret;
}
		...
		pte_t *ptep)
{
	...
	ret = PVOP_CALL3(pte_t, mmu.ptep_get_and_clear, mm, addr, ptep);
...
static inline void ptep_set_wrprotect(struct mm_struct *mm, unsigned long addr,
		pte_t *ptep)
{
	PVOP_VCALL3(mmu.ptep_set_wrprotect, mm, addr, ptep);
}

static inline int pmdp_set_access_flags(unsigned long address, pmd_t *pmdp,
		pmd_t entry, int dirty)
{
	int ret;
	ret =  PVOP_CALL4(int, mmu.pmdp_set_access_flags, address,
			pmdp, native_pmd_val(entry), dirty);
	return ret;
}

static inline int pudp_set_access_flags( unsigned long address, pud_t *pudp,
		pud_t entry, int dirty)
{
	int ret;
	ret =  PVOP_CALL4(int, mmu.pudp_set_access_flags, address,
			pudp, native_pud_val(entry), dirty);
	return ret;
}
...
{
	int ret;
	ret =  PVOP_CALL3(int, mmu.pmdp_test_and_clear_young, vma, addr, pmdp);
	return ret;
}
...
{
	int ret;
	ret =  PVOP_CALL3(int, mmu.pudp_test_and_clear_young, vma, addr, pudp);
	return ret;
}
		...
		unsigned long addr, pmd_t *pmdp)
{
	...
	ret = PVOP_CALL3(pmdval_t, mmu.pmdp_huge_get_and_clear, mm, addr, pmdp);
		...
		unsigned long addr, pud_t *pudp)
{
	...
	ret = PVOP_CALL3(pudval_t, mmu.pudp_huge_get_and_clear, mm, addr, pudp);
...
static inline void pmdp_set_wrprotect(struct mm_struct *mm, unsigned long addr,
		pmd_t *pmdp)
{
	PVOP_VCALL3(mmu.pmdp_set_wrprotect, mm, addr, pmdp);
}
		...
		unsigned long address, pmd_t *pmdp, pmd_t pmd)
{
	...
	ret = PVOP_CALL4(pmdval_t, mmu.pmdp_establish, vma, address, pmdp,
			native_pmd_val(pmd));
